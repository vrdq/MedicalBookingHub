...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const ThemeToggle = ({ className = "" }: { className?: string }) => {
    ...
    if (!mounted) {
      return (
        <div className={`p-2 rounded-md border border-border w-8 h-8 ${className}`} />
      );
    }
    ...
    return (
      <button
        ...
        className={`p-2 rounded-md border border-border hover:bg-muted transition-colors text-muted-foreground hover:text-foreground cursor-pointer ${className}`}
      >
