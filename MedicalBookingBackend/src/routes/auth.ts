...
router.post("/auth/register", async (req, res): Promise<void> => {
  ...
  // Send verification email (awaited to prevent Vercel from freezing connection mid-send)
  try {
...
const RegisterDoctorBodySchema = z.object({
  ...
  type: z.enum(["psychiatrist", "psychologist"]),
  ...
  sessionType: z.enum(["individual", "group", "both"]).optional(),
});

router.post("/auth/register-doctor", async (req, res): Promise<void> => {
  const parsed = RegisterDoctorBodySchema.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.issues.map(i => i.message).join(", ") });
    return;
  }

  const { firstName, lastName, email, password, phone, specialty, type, gender, price, bio, yearsExperience, languages, sessionType, paymentInfo } = parsed.data;
